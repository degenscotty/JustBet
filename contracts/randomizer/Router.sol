...
contract RandomizerRouter is Access, IRandomizerRouter {
    ...
    event Triggered(uint256 indexed requestId);

    event ReRequested(uint256 indexed requestId, address consumer, address provider);
    ...
    function request(
        uint32 _count,
        uint256 _minConfirmations
    ) external onlyConsumer returns (uint256 requestId_) {
    ...
    function trigger(uint256 _requestId) external onlyNotFilled(_requestId) {
    ...
    function reRequest(uint256 _requestId) external onlyNotFilled(_requestId) {
        Request memory request_ = requests[_requestId];
        IRandomizerProvider(request_.provider).request(
            _requestId,
            request_.count,
            request_.minConfirmations
        );
    ...
    function response(
        ...
        emit Fulfilled(_requestId, address(request_.consumer), _msgSender(), _rngList);
    }
}
